...
\begin{itemize}
    ...
    \item New Section~4.1: Fifth coordination mode (Sophisticated mutual phantom inference) where each agent maintains a model of the other's decision process.
    \item New Section~5.7: Cost-aware planning results with predictive $B$ matrices.
...
EcoNet achieves a 6.1\% cost reduction and 23.0\% GHG reduction relative to the No-HEMS baseline, while maintaining substantially better thermal comfort. The Rule-Based baseline (charge during off-peak, discharge during peak, thermostat at fixed setpoints) performs worse on cost because it does not adapt to context. Figure~9 provides a visualization of these comparisons.
...
\begin{quote}
``Simulation results demonstrate that EcoNet achieves a 6.1\% reduction in energy costs and a 23.0\% reduction in grid-associated greenhouse gas emissions relative to unmanaged operation, while reducing thermal comfort deviations by 66\%. These results are robust across four distinct climate zones tested with real meteorological data.''
\end{quote}
...
\begin{itemize}
    \item \textbf{No-HEMS}: No active management; thermostat remains off, battery unused. This represents the worst case for comfort and a passive baseline for cost (household draws all energy from grid at prevailing rates).
...
--- & Sophisticated mode & Section~4.1 (new) \\
