...
\maketitle
\begin{abstract}
This paper introduces EcoNet, a multi-agent active inference framework
for household energy management that coordinates a Thermostat Agent and
a Battery Agent to balance energy cost, thermal comfort, and greenhouse
gas emissions under uncertainty. Unlike reinforcement learning (RL),
which requires extensive offline training, or model predictive control
(MPC), which assumes deterministic dynamics, EcoNet uses the free
energy principle to unify perception, learning, and action. A key
contribution is the use of \emph{predictive transition matrices} that
incorporate weather forecasts, tariff schedules, and occupancy calendars
into the generative model at every time step, combined with ToU-aware
dynamic preference vectors that adapt to context. We investigate
coordination through federated belief sharing
\cite{friston2024federated} and a Theory of Mind (ToM) mode based on
sophisticated inference \cite{pitliya2025theory}, where agents maintain
phantom models of each other's decision processes. Across four climate
scenarios (5 seeds each), EcoNet achieves costs within 2--4\% of a
perfect-foresight Oracle, generalizes across diverse real-weather
conditions without retraining, and maintains stable or comparable performance under
forecast noise ($\sigma$ up to 4$^{\circ}$C), with MPC degrading by
3.8--8.5\%. A systematic RL comparison shows that EcoNet achieves
near-Oracle performance with zero training, whereas tabular Q-learning
requires 20,000+ episodes and still incurs 14--26\% higher costs in
temperate and hot climates, with 11.8\% additional degradation when
transferring across climates.
\end{abstract}
...
The preferences and goals of energy management can be conditional
...
Active inference differs from existing approaches by providing
a single framework that integrates planning under uncertainty,
...
natively support online parameter learning or exploration. EcoNet's active inference agents, in contrast, maintain probabilistic beliefs
over hidden states, learn transition dynamics via Dirichlet concentration
updates, and select actions by minimizing expected free energy. This quantity
automatically balances goal-seeking (exploitation) with uncertainty reduction
(exploration). The preference structure can also be made context-dependent
(e.g., the Battery Agent's preferences shift between charging and discharging
based on the current ToU period), which handles
conditional goals that would require ad hoc reward shaping in RL.
...
parameters online. We further investigate coordination through
federated belief sharing and Theory of Mind (phantom inference),
building on a baseline of ToU-aware dynamic preference vectors
shared by all agents. The coordinated modes additionally share beliefs
or maintain phantom models of each other, achieving near-optimal
performance across cost, comfort, and emissions through decentralized
coordination
...
analyses, a coordination study, uncertainty robustness analysis, and
a reinforcement learning comparison, and discuss outcomes and future work.
...
Despite their potential benefits, neither MPC nor RL has seen wide
...
used as source for transfer learning and applied to other target
buildings.
...
generative probabilistic model that accounts for uncertainty.
...
Active inference is also apt for multiple constraint satisfaction
problems, by specifying preferences in terms of probability distributions
over outcomes. Active inference is well suited for settings where an agent
...
This is expected, as the active inference model encapsulates
...
This fits within the active inference framework:
...
because (i) the EFE weights near-term predictions more
...
direct awareness of energy costs. To enable cost--comfort
...
discharge. The preferences
legitimately change with context: they encode the household's conditional
...
A central question in multi-agent HEMS is how agents should coordinate their
actions. We investigate five coordination architectures, three of which
serve as baseline modes and two of which introduce genuinely distinct
coordination mechanisms:

\textbf{Independent / Aligned / Hierarchical (baseline modes).}
Each agent dynamically adjusts its $C$ vector based on local
environmental signals (ToU period, occupancy). During peak periods, the
Battery Agent's preference for discharge increases; the Thermostat Agent
relaxes comfort preferences when unoccupied. In the \emph{Independent}
mode, there is no inter-agent coordination signal beyond the cascading
update structure described above. The \emph{Aligned} mode is
architecturally identical but is conceptually intended for scenarios
where shared context signals are available. The \emph{Hierarchical}
mode introduces a meta-agent that can explicitly set $C$ vectors, but
since both agents already adapt their preferences to the same
environmental signals, the meta-agent provides no additional benefit in
the current single-household setting. Empirically, all three modes
produce identical results (Tables~\ref{tab:coordination}
and~\ref{tab:oracle_gap}); the ToU-aware dynamic $C$
vectors, rather than the coordination architecture, are the primary
driver of near-optimal performance. We retain these three modes to
delineate the design space and to provide a clear baseline against which
the genuinely distinct coordination mechanisms (Federated and ToM) can
be evaluated.
...
raw states for decentralized consensus formation. This mode extends
...
within pymdp's standard EFE computation:
\begin{equation}
...
effective social preference weight, so that agents fall back to
...
(Equation~\ref{eq:phantom_B}), so the battery can anticipate
...
so that they can anticipate tariff transitions across the full planning
...
In this section, we present results for nine experimental analyses:
...
(8) an uncertainty robustness analysis under increasing forecast noise; and
(9) a reinforcement learning comparison evaluating sample efficiency and
...
period lasts for three hours. Policy length is six steps (left panel)
...
(policy length 6, single seed). The multi-seed evaluation
...
and does not handle model uncertainty or online learning.
...
quantify uncertainty through posterior beliefs, and can learn
unknown parameters online, advantages that matter more
...
An important question for any HEMS approach is whether it generalizes across
...
costs, due to the low carbon intensity of hydroelectric generation.
...
experiments (Section~\ref{sec:coordination_results}). Neither mode requires any retraining or parameter adjustment
...
which supports the use of synthetic data as a reasonable proxy for
...
balance is satisfied exactly (to machine precision): no energy is created or destroyed by the control logic.
...
Among the AIF modes, all five produce nearly identical costs:
Independent, Aligned, Hierarchical, and ToM at \$19.58, and Federated
at \$19.54 (10.3\% above Oracle). The Federated mode achieves a
marginal improvement through explicit belief sharing, which allows the
Battery Agent to anticipate the Thermostat's comfort state.
The near-equivalence of all modes confirms that the ToU-aware dynamic
$C$ vectors alone account for the cost performance; neither
centralized coordination, belief sharing, nor phantom inference adds
substantial benefit in the single-household setting.
The Pareto frontier (Figure~\ref{fig:pareto}) illustrates the
cost--comfort trade-off across all conditions.
The tabular RL baseline performs poorly (\$29.36) because learning effective energy management policies from scratch
with limited training data and coarse discretization is difficult.
...
\midrule
EcoNet: Independent  & 19.58 & 108.6 & 35.4 \\
EcoNet: Aligned      & 19.58 & 108.6 & 35.4 \\
EcoNet: Hierarchical & 19.58 & 108.6 & 35.4 \\
EcoNet: Federated    & 19.54 & 108.6 & 35.3 \\
EcoNet: ToM          & 19.58 & 108.6 & 35.4 \\
\bottomrule
\end{tabular}
\end{table}
...
\par\end{centering}
\caption{Cost vs. Comfort Pareto frontier. RL and No-HEMS are omitted
to focus on the interesting region; their values appear in
Table~\ref{tab:coordination}. All non-Federated AIF modes overlap at
a single point and are consolidated.}\label{fig:pareto}
\end{figure}
...
all five AIF modes achieve nearly identical costs (\$19.54--\$19.58,
approximately 10\% above Oracle). The ToU-aware dynamic
preferences drive performance rather than the coordination architecture.
...
meaningful uncertainty in the thermostat's comfort assessment: agents
...
(the ToM mode described in Section~\ref{sec:coordination}) in which the Thermostat Agent also uses a
...
First, all AIF agents (Independent and Aligned produce identical
results) closely track the Oracle across all four climates. The
average cost gap is \$0.46 (2.0\% of Oracle cost) and the average GHG
gap is 0.90~kg (2.3\% of Oracle GHG). The predictive $B$ matrices and
ToU-aware dynamic $C$ vectors enable the agent to approach
globally-optimal battery scheduling despite having only an 8-hour
rolling horizon. Results are stable across seeds, with
...
\$0.21), showing that the generative model adapts to diverse
...
due to its strong comfort preference in the $C$ vector.
...
near-Oracle performance; the planning problem is
well-conditioned when given sufficient lookahead. In London winter, MPC slightly undercuts the Oracle on cost
(\$33.78 vs.\ \$34.01) because it re-plans at each step rather than
optimizing globally. However, MPC's lower cost comes at worse comfort
(123.7 vs.\ 127.2~$^{\circ}$C$\cdot$h), which represents a different
trade-off on the Pareto frontier rather than true dominance. MPC requires a complete model of system dynamics and
deterministic optimization, whereas the AIF agent learns its generative
model structure and provides uncertainty quantification through
posterior beliefs. The tabular RL baseline performs poorly because coarse
state discretization limits its ability to learn effective policies from scratch.

Fourth, all AIF modes (Independent, Aligned, Hierarchical, Federated, ToM)
...
which shows that the dynamic preference mechanism, rather than
...
In Phoenix, where cooling runs near-continuously,
...
because London's more variable thermal load means forecast errors
...
as expected since both modes use the same ToU-aware dynamic $C$
...
achieves this bound because the agents' generative models are robust enough
...
($\sigma=0$) as forecast noise increases, for Oracle, MPC, and AIF Aligned.
AIF in Phoenix shows zero degradation, while MPC degrades by
3.8\%.}\label{fig:uncertainty_robustness}
\end{figure}
...
AIF Aligned is included as a reference (zero-training baseline).
...
\textbf{Cross-climate transfer.} An advantage of model-based approaches
...
due to overfitting to London's thermal dynamics and ToU patterns.
...
Oracle and AIF Aligned reference costs. Even at
...
and as DER penetration in distribution grids grows,
coordinated energy management becomes more important for grid
flexibility and stability. Coordination is needed at multiple scales:
between devices within a household, between households in a
neighborhood, and across regions of the grid. Intelligent management involves forecasts (of weather, solar
...
promising, because an active inference agent can choose actions to resolve
...
Oracle on average (maximum gap 4.2\% in London summer). The cost
advantage over a fixed-thermostat baseline reaches 17\% in mild climates.
...
the planning problem is well-conditioned when given
sufficient lookahead, and the advantage of AIF lies in its probabilistic
...
The Federated mode's pattern is consistent with federated inference
theory \cite{friston2024federated}. Sharing posterior beliefs (full
probability distributions, not point estimates) enables agents to form
decentralized consensus about shared world states. The magnitude of
the benefit depends on the degree of coordination opportunity in the
environment. The auditory A matrix formulation
(described in Section~\ref{sec:coordination}) ensures that social preferences
emerge within the standard EFE computation
...
(Equation~\ref{eq:reliability}) provides a mechanism for trust
calibration, letting agents discount unreliable reports.
...
demonstrates how to incorporate forecast information into
...
encodes forecast uncertainty, with the spread parameter
$\sigma$ controlling confidence in the weather prediction.
...
construction means that anticipatory coordination emerges
...
The three baseline
architectures (Independent, Aligned, Hierarchical) produce identical
results, demonstrating that the ToU-aware dynamic $C$ vectors, rather
than coordination architecture, are the primary mechanism driving
near-optimal performance in the single-household setting. The Federated
and ToM modes represent two genuinely distinct coordination mechanisms:
explicit belief sharing and implicit phantom inference. Both achieve
comparable performance through different mechanisms: the Federated mode
requires a communication channel while the ToM mode requires shared
knowledge of the other agent's generative model structure. The equivalence of the baseline modes suggests that coordination
benefits will become more pronounced in multi-household settings.
In such settings, agents cannot observe the same environmental signals
and must explicitly communicate or model each other to anticipate
correlated behavior \cite{heins2024collective,heins2023spin,pitliya2025theory}.

The climate sensitivity analysis (Section~\ref{sec:climate}) shows
that EcoNet generalizes across diverse climate conditions without
retraining. This is a direct consequence of online adaptation through
belief updating, in contrast to RL approaches that require re-training
for each new climate. The agent adapts its behavior automatically
(e.g., prioritizing cooling in Phoenix summers and heating in Montreal
winters). The use of real weather data from the IEM ASOS network
provides confidence that performance would be maintained under realistic
...
The RL comparison (Section~\ref{sec:rl_comparison}) reveals three
advantages of active inference. First, AIF
...
seeds ($\pm$\$0.44 vs.\ $\pm$\$2.37), which indicates the stability of
model-based planning compared to value-function estimation. These
results suggest that model-based active inference outperforms
model-free RL for energy management, where the physical dynamics
are known and can be encoded directly in the generative model.
...
Multiple studies suggest that HEMS can reduce energy costs, conserve
resources, reduce greenhouse gas emissions, and increase grid stability
\cite{khabbazi2025,Mahapatra2022-zp,Martinez-2021,Tuomela-2021}.
For example, Gualandri and Kuzior \cite{Gualandri-2023} show that
increased HEMS adoption could substantially reduce residential energy
consumption in Italy, helping the country meet 2030 European energy
targets. Future HEMS adoption depends on choices made by manufacturers,
utilities, consumer groups, end users, and policymakers, which in turn
depends on cooperation between these entities
...
building clusters would expand our two-agent model to a
many-agent approach. The belief-sharing architecture presented here
provides a foundation for such scaling, as each agent need only
...
A matrix itself, so that agents adaptively model each other's
...
by having the phantom learn from observation (i.e., updating
...
comparison with Oracle, MPC, and RL baselines shows active
inference's advantages: online learning without offline
training, built-in uncertainty quantification, and interpretable
preference structures through $C$ vectors. Uncertainty robustness experiments confirm that AIF maintains stable
costs under forecast noise (0\% degradation in Phoenix at
$\sigma = 4$~$^{\circ}$C, vs.\ 3.8\% for MPC). A systematic RL
comparison shows that AIF achieves near-Oracle performance with zero
training, while RL requires 20,000+ episodes and still incurs
14--26\% higher costs with 11.8\% cross-climate transfer degradation.

The framework provides a foundation for scaling to multi-household
neighborhoods and integrating additional DER devices, using both
