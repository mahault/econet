...
based on the current ToU period), providing a principled mechanism for handling
...
and~\ref{tab:oracle_gap}), confirming that the ToU-aware dynamic $C$
...
approximately 10\% above Oracle), confirming that the ToU-aware dynamic
...
(123.7 vs.\ 127.2~$^{\circ}$C$\cdot$h), reflecting a different
...
posterior beliefs. The tabular RL baseline performs poorly, reflecting the
