...
(MPC), which assumes deterministic dynamics, EcoNet leverages the free
...
The preference structure of active inference agents can also be made
...
First, all AIF agents (Independent and
...
well-conditioned when given sufficient lookahead. In London winter,
MPC slightly undercuts the Oracle on cost (\$33.78 vs.\ \$34.01)
because both use backward induction with the same multi-objective
function (cost $+$ comfort$\,\times\,$0.3), but the Oracle optimizes
globally while MPC re-plans at each step; MPC's lower cost comes at
worse comfort (123.7 vs.\ 127.2~$^{\circ}$C$\cdot$h), reflecting a
different trade-off on the Pareto frontier rather than a true
dominance. MPC requires a complete model of system dynamics and
...
it becomes more important for grid flexibility and stability
...
knowledge of the other agent's generative model structure. The
equivalence of the baseline modes suggests that coordination benefits
will become more pronounced in multi-household settings, where agents
cannot observe the same environmental signals and must explicitly
communicate or model each other to anticipate correlated behavior. This finding is consistent with work on
