...
by approximately 6\% compared to a fixed-thermostat baseline (which lacks
battery management) and by 52\% compared to
...
mappings in each agent's generative model. In a synthetic benchmark
scenario, this belief-sharing mechanism
...
objectives. Across real-weather scenarios, cost savings are more modest
(0.6\% aggregate) but comfort improvements persist. In the sophisticated mode, a cost-aware Thermostat Agent
...
Unless otherwise noted, all experiments use a policy length of 6 steps
(12 hours), precision parameter $\gamma = 16$, and 2-hour time steps.
The deterministic and baseline comparison experiments use 2-day
synthetic data; the coordination, climate, and cost-aware experiments
use 7-day data with online $B$-matrix learning enabled. The policy
length of 4 steps used in some coordination experiments is noted
...
knowledge of the system model. Matches the AIF agents' planning horizon.
...
EcoNet's performance. MPC with a 6-step horizon (matching the AIF
...
Third, the No-HEMS baseline (which now includes a fixed thermostat
...
battery's round-trip efficiency ($\eta = 0.9$ one-way, 81\% round-trip):
battery operation incurs energy losses, but enables TOU arbitrage
(charging off-peak, discharging on-peak) that reduces cost despite these
losses. The Oracle and Aligned agent use the battery to \emph{shift}
energy consumption from high-ToU to low-ToU periods, achieving net
cost savings that outweigh the efficiency penalty. The MPC baseline
achieves near-Oracle performance with the same 6-step horizon as the AIF
agents, but requires a complete model of system dynamics; the RL baseline
achieves reasonable performance after training but lacks uncertainty
